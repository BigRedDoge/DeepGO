...
class CSGONet(nn.Module):
    def __init__(self):
        super().__init__()
        self.device = select_device('cuda:0' if torch.cuda.is_available() else 'cpu')
        self.weights = r'C:\Users\Sean\Documents\CS-AI\yolov5\weights\326_head_body.pt'
    ...
    def forward(self, x):
        ...
        x = torch.stack([self.yolo(each) for each in xs], dim=1)
        print('feature shape: ', x.shape)
        x = x.float()
         
        layer_output_list, last_state_list = self.convlstm(x)
        x = layer_output_list[-1].half()
        x = torch.unbind(x, dim=1)[-1]
        print('lstm output shape: ', x.shape)

        # flatten all dims except batch
        x = torch.flatten(x, 1) # ([time step size, 2560])
        print('flatten shape: ', x.shape)

        x = self.leaky_relu(self.fc1(x))  # ([time step size, 36864])
